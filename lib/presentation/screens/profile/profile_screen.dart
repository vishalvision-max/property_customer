...
class _ProfileScreenState extends ConsumerState<ProfileScreen> {
  String _normalizeImage(String raw) {
    ...
    return Uri.parse('https://propertysearch.visionvivante.in').resolve('/storage/$v').toString();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final displayName = (owner?.name.trim().isNotEmpty ?? false)
        ? owner!.name.trim()
        : ((user == null || user.name.trim().isEmpty) ? 'Guest' : user.name.trim());
    final displayEmail = (owner?.email.trim().isNotEmpty ?? false)
        ? owner!.email.trim()
        : ((user == null || user.email.trim().isEmpty) ? '-' : user.email.trim());
                  ...
                  children: [
                    Container(
                      ...
                      child: ownerImage.isEmpty
                          ? const Icon(Icons.person_rounded, color: _kPrimary, size: 30)
                          : CachedNetworkImage(
                              imageUrl: ownerImage,
                              fit: BoxFit.cover,
