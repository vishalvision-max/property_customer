...
class ScheduledVisitsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ScheduledVisitsScreen> createState() => _ScheduledVisitsScreenState();
}
          ...
          Expanded(
            child: visitsAsync.when(
              data: (visits) {
                ...
                final sortedVisits = List<ScheduledVisit>.from(visits)..sort((a, b) {
                  final da = DateTime.tryParse('${a.scheduledDate} ${a.scheduledTime}') ?? DateTime.tryParse(a.scheduledDate) ?? DateTime(1970);
                  final db = DateTime.tryParse('${b.scheduledDate} ${b.scheduledTime}') ?? DateTime.tryParse(b.scheduledDate) ?? DateTime(1970);
              ...
              error: (err, stack) => Center(child: Text('Error loading visits')),
            ),
          ),
        ],
      ),
...
class _VisitCard extends StatelessWidget {
  ...
  (Color bg, Color fg, String text) _statusConfig(String st) {
    ...
    if (status == 'confirmed') return (const Color(0xFFD1FAE5), const Color(0xFF059669), 'Confirmed');
    if (status == 'pending') return (const Color(0xFFFEF3C7), const Color(0xFFD97706), 'Pending');
    if (status == 'scheduled') return (const Color(0xFFE0F2FE), const Color(0xFF0284C7), 'Scheduled');
    return (Colors.grey[200]!, Colors.grey[700]!, status.toUpperCase());
  }

  @override
  Widget build(BuildContext context) {
    final conf = _statusConfig(visit.status.isEmpty ? 'scheduled' : visit.status);
    ...
    final monthStr = date != null ? DateFormat('MMM').format(date).toUpperCase() : 'JUN';
    ...
    String timeStr = visit.scheduledTime.isNotEmpty ? visit.scheduledTime : '11:00 AM';
        ...
        children: [
          ...
          Column(
            children: [
              ...
              Text(
                dayStr,
                style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: _kTextDark),
              ),
              Text(
                monthStr,
                ...
                Row(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(
                        conf.$3,
                        style: TextStyle(color: conf.$2, fontSize: 10, fontWeight: FontWeight.bold),
                      ),
                    ),
                  ],
                ),
                  ...
                  children: [
                    Expanded(
                      child: OutlinedButton(
                        ...
                        child: const Text('Reschedule', style: TextStyle(color: _kPrimary, fontSize: 12, fontWeight: FontWeight.bold)),
                      ),
                    ),
                    ...
                    Expanded(
                      child: ElevatedButton(
                        ...
                        child: const Text('Call Agent', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)),
                      ),
                    ),
                  ],
                ),
